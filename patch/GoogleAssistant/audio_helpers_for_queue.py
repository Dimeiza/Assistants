...
class SoundDeviceStream(object):
    ...
    """
    def __init__(self, sample_rate, sample_width, block_size, flush_size, isOutput):
        if sample_width == 2:
            audio_format = 'int16'
        else:
            raise Exception('unsupported sample width:', sample_width)
    ...
    def flush(self):
        if self._flush_size > 0 and self._isOutput==True:
...
def main(record_time, audio_sample_rate, audio_sample_width,
    ...
    source_audio_device = SoundDeviceStream(sample_rate=audio_sample_rate,
                                     sample_width=audio_sample_width,
                                     block_size=audio_block_size,
                                     flush_size=audio_flush_size,
                                     isOutput=False)
                                     
    sink_audio_device = SoundDeviceStream(sample_rate=audio_sample_rate,
                                     sample_width=audio_sample_width,
                                     block_size=audio_block_size,
                                     flush_size=audio_flush_size,
                                     isOutput=True)
                                                          
    stream = ConversationStream(source=source_audio_device,
                                sink=sink_audio_device,
